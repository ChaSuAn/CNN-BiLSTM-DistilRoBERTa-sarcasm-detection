...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings(config).cuda()
    ...
    def forward(self,input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2).cuda()
...
class BERT(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor ):
        ...
        attention_mask = torch.zeros_like(tensor_list).long().cuda()
        if self.enc_num > 0:
            all_encoder_layers = self.bert(tensor_list, token_type_ids=None, attention_mask=attention_mask)
            # use the output of the X-th transformer encoder layers
            xs = all_encoder_layers[self.enc_num - 1]
        else:
            xs = self.bert.embeddings.word_embeddings(tensor_list)
